<template>
  <div class="catalog">
    <h1>ФЕНТЕЗІ</h1>
    <p class="start">Небезпечні пригоди і чарівні істоти чекають на свого читача, 
                    щоб віднести його у створений автором світ. Якщо й тікати від реальності, 
                    то тільки у книги! Будьте тим, ким завжди мріяли стати: ельфом, 
                    чарівником чи тролем — все обмежується лише вашою уявою.</p>
...
export default {
  ...
  methods: {
    fetchBooks() {
      axios.get('http://localhost/Book-Store/database/getBooksByGenre.php?genre_id=4')
        .then(response => {
